...
define_WGD_nodes <-function(phy,clade){
	if (clade=='MH'){
		...
		if (length(M)+ length(H)>=2){
			...
			if ((is.monophyletic(phy,c(M,H))) & ((length(M)>=2) | (length(H)>=2))){	#((M,H),(M,H))
				MH_node=paste('MRCA MH ',paste(M,H,collapse =' '),';',sep='')
				write(MH_node,ctr_filename,append=T)
			}
		}
	}
	if (clade=='CG'){
		...
		if (length(C)+ length(G)>=2){
			...
			if ((is.monophyletic(phy,c(C,G))) & ((length(C)>=2) | (length(G)>=2))){	#((C,G),(C,G))
				CG_node=paste('MRCA CG ',paste(C,G,collapse =' '),';',sep='')
				write(CG_node,ctr_filename,append=T)
			}
		}
	}
	if (clade=='PS'){
		...
		if (((any(startsWith(clade1,'Fla'))) & (any(startsWith(clade2,'Fla')))) | ((any(startsWith(clade1,'SalixSu_'))) & (any(startsWith(clade2,'SalixSu_')))) | ((any(startsWith(clade1,'Salix_'))) & (any(startsWith(clade2,'Salix_')))) |((any(startsWith(clade1,'Pop'))) & (any(startsWith(clade2,'Pop'))))){
			...
			PS_node=paste('MRCA PS ',paste(sample(clade1,1),sample(clade2,1)),';',sep='')
			write(PS_node,ctr_filename,append=T)
		}
	...
	if (clade=='CM'){
		...
		if (length(member)>=2){
			...
			if (((any(startsWith(clade1,'Calo'))) & (any(startsWith(clade2,'Calo')))) | ((any(startsWith(clade1,'Mamm'))) & (any(startsWith(clade2,'Mamm'))))){
				CM_node=paste('MRCA CM ',paste(sample(clade1,1),sample(clade2,1)),';',sep='')
				write(CM_node,ctr_filename,append=T)
			}
			else{
	...
	if (clade=='Dry'){
		...
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Dry ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Vio'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Vio ',paste(taxa),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Pas'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Pas ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='End'){
		taxa=phy$tip.label[which(startsWith(phy$tip.label,'End'))]
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA End ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Ery'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Ery ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Rhi'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Rhi ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Sau'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Sau ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Bis'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Bis ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Pod'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Pod ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Hyp'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Hyp ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Lin'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Lin ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
		if ((length(taxa)==4) | (length(taxa)==3)){
			if (length(taxa)==4){
				node=getMRCA(phy,taxa)
				daughters=which(phy$edge[,1]==node)
				clade1 = tips(phy,phy$edge[daughters[1],2])
				clade2 = tips(phy,phy$edge[daughters[2],2])
				if (length(clade1)==2){
					#((a1,a2),(b1,b2))
					node=paste('MRCA Lin_old ',paste(clade1,clade2,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
					node=paste('MRCA Lin_new ',paste(clade1,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
					node=paste('MRCA Lin_new ',paste(clade2,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
				}
				else {
					##(ax,(ay,(b1,b2)))
					...
					node=getMRCA(phy,sub2)
					daughters=which(phy$edge[,1]==node)
					clade3 = tips(phy,phy$edge[daughters[1],2])
					clade4 = tips(phy,phy$edge[daughters[2],2])
					if (length(clade3)>length(clade4)){sub3=clade3}
					else{sub3=clade4}
					node=paste('MRCA Lin_old ',paste(sub1,sub3,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
					node=paste('MRCA Lin_new ',paste(sub3,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
				}
			}
			else{
				...
				if (length(clade1)<length(clade2)){
					...
					node=paste('MRCA Lin_old ',paste(clade2,clade1,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
					node=paste('MRCA Lin_new ',paste(clade1,collapse =' '),';',sep='')
					write(node,ctr_filename,append=T)
				}
	...
	if (clade=='Och'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Och ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Gal'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Gal ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
	if (clade=='Bhe'){
		...
		taxa=return_monophyletic_gr(phy,taxa) #delete up to 3 outliers
		if ((length(taxa)>=2) & (is.monophyletic(phy,taxa))){
			node=paste('MRCA Bhe ',paste(taxa,collapse =' '),';',sep='')
			write(node,ctr_filename,append=T)
		}
	}
}
...
for (i in treefile_names){ 
	...
	ctr_filename=paste(id[[1]][1],'_',id[[1]][2],'.nex',sep='')
	...
	#define WGD nodes #sapply?
	for (clade in c('MH','CM','CG','PS','Dry','Vio','Pas','End','Ery','Rhi','Sau','Bis','Pod','Hyp','Lin','Och','Gal','Bhe')){define_WGD_nodes(tree,clade)}
