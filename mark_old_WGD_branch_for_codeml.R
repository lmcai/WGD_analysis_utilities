...
define_WGD_nodes <-function(phy,clade){
	...
	if (clade=='CM'){
		#taxa=phy$tip.label[which(startsWith(phy$tip.label,'Mamm'))]
...
#Vio_gene=read.table('MioPlio.Vio.gene',header=F)
#Lin_gene=read.table('MioPlio.Vio.gene',header=F)
#Gal_gene=read.table('MioPlio.Gal.gene',header=F)
#CM_gene=read.tabble('MioPlio.CM.gene',header=F)
#End_gene=read.table('MioPlio.End.gene',header=F)

Rhi_gene=read.table('PalEoc.Rhi.gene',header=F)
Pas_gene=read.table('PalEoc.Pas.gene',header=F)
Och_gene=read.table('PalEoc.Och.gene',header=F)
Ery_gene=read.table('PalEoc.Ery.gene',header=F)
CM_gene=read.table('PalEoc.CM.gene',header=F)
CG_gene=read.table('PalEoc.CG.gene',header=F)
Lin_gene=read.table('PalEoc.Lin.gene',header=F)
...
for (i in treefile_names){ 
	...
	new_filename=paste(id[[1]][1],'.marked.old.tre',sep='')
	...
	contain_young_WGD=0
	#'Rhi','Pas','Och','Ery','CM','CG','Lin'
	if (id[[1]][1] %in% Rhi_gene$V1){
		tree=define_WGD_nodes(tree,'Rhi')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% Pas_gene$V1){
		tree=define_WGD_nodes(tree,'Pas')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% Och_gene$V1){
		tree=define_WGD_nodes(tree,'Och')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% Ery_gene$V1){
		tree=define_WGD_nodes(tree,'Ery')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% CM_gene$V1){
		tree=define_WGD_nodes(tree,'CM')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% CG_gene$V1){
		tree=define_WGD_nodes(tree,'CG')
		contain_young_WGD=1
	}
	if (id[[1]][1] %in% Lin_gene$V1){
		tree=define_WGD_nodes(tree,'Lin')
		contain_young_WGD=1
	}
	if (contain_young_WGD==1){write.tree(tree,new_filename)}
	#conduct cv	
